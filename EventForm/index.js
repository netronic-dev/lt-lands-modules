...
export function EventForm(props) {
  ...
  return (
    <section className={style.event_form}>
      {isThankYouActive ? <ThankYou /> : <Form thankYou={onThankYouView} />}
    </section>
  );
}
...
function Form(props) {
  ...
  let equipmentTypeValue = `${formik.values.isOutdoor ? "Outdoor " : ""}${
    formik.values.isIndoor ? "Indoor " : ""
  }${formik.values.isVR ? "VR " : ""}${
    formik.values.isBunkers ? "Bunkers " : ""
  }`;

  let modelValue = `${formik.values.isBasic ? "Basic " : ""}${
    formik.values.isStandard ? "Standard " : ""
  }${formik.values.isPremium ? "Premium " : ""}${
    formik.values.isPro ? "Pro " : ""
  }`;

  return (
    <form className={style.form} onSubmit={formik.handleSubmit}>
      <h1 className={style.title}>Event form</h1>
      <Input
        name="name"
        type="text"
      ...
      />
      <h2 className={style.title}>Equipment type</h2>
      <div className={style.checkboxes}>
        ...
        />
        <CheckBox title="VR" active={formik.values.isVR} onClick={onVRChange} />
        <CheckBox
          title="Bunkers"
      ...
      <h2 className={style.title}>Model</h2>
      <div className={style.checkboxes}>
        <CheckBox
          title="Basic"
...
function ThankYou() {
  return (
    <div>
      <h1 className={style.title}>Thank you, the request has been sent</h1>
    </div>
  );
}
...
async function postEventData(data) {
  ...
  axios
    .post(
      "https://sheet.best/api/sheets/a029e18d-160e-4ece-8e2e-8c8ac93d67ce",
      {
